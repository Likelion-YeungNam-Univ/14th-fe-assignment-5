    ...
    <div className="sticky top-0 z-50 bg-[#0f0f0f]/90 backdrop-blur-md flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-2">
        <div className="flex items-center gap-3 mt-2">
          <img src={menubar} alt="menubar" className="w-5 h-5 invert" />
        ...
        <div className="absolute left-1/2 -translate-x-1/2 flex items-center gap-2 mt-3">
          <div className="flex items-center border border-gray-600 rounded-full overflow-hidden w-[35vw]">
            <input
              type="text"
              placeholder="검색"
              className="px-4 py-1 w-full outline-none bg-transparent text-white"
            />
            <button className="px-4 py-1 bg-[#2d2d2d] border-l border-gray-600 text-white">
              <img src={searchIcon} alt="search" className="w-7 h-7 invert" />
            </button>
          </div>
          <div className="p-2 bg-[#2d2d2d] rounded-full cursor-pointer hover:bg-gray-600">
            <img src={mic} alt="mic" className="w-7 h-7 invert" />
          </div>
        </div>
      ...
      <div className="flex space-x-2 px-4 py-2 overflow-x-auto scrollbar-hide mt-1">
        {categoryList.map((category) => (
          <div
            key={category.id}
            className="bg-[#2d2d2d] font-bold text-white py-1 px-4 text-sm rounded-lg whitespace-nowrap cursor-pointer hover:bg-[#3d3d3d] transition-colors"
          >
            {category.category}
          </div>
        ))}
